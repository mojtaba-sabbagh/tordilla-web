...
export async function generateMetadata({ params, searchParams }: PaginatedPageProps) {
  ...
  const description = locale === "fa" ? `مقالات آموزشی ترددیلا - صفحه ${pageNum}` : `Tordilla educational articles - Page ${pageNum}`;
...
export default async function PaginatedBlogPage({ params, searchParams }: PaginatedPageProps) {
  ...
  return (
    <main className="paginated-blog-page" dir={locale === "fa" ? "rtl" : "ltr"}>
      <style>{`
        .paginated-blog-page { background: #fdf8f3; min-height: 100vh; }
        .blog-hero { position: relative; background: linear-gradient(135deg, #8f1d1d 0%, #5c1111 100%); padding: 80px 24px 100px; text-align: center; overflow: hidden; }
        .blog-hero::before, .blog-hero::after { content: ''; position: absolute; border-radius: 50%; pointer-events: none; }
        .blog-hero::before { width: 360px; height: 360px; top: -120px; left: -80px; background: rgba(255,255,255,0.05); }
        .blog-hero::after { width: 280px; height: 280px; bottom: -100px; right: -60px; background: rgba(255,255,255,0.04); }
        .blog-hero-badge { display: inline-flex; align-items: center; gap: 6px; padding: 5px 18px; border-radius: 9999px; border: 1.5px solid rgba(255,255,255,0.28); background: rgba(255,255,255,0.1); color: #fff; font-size: 13px; font-weight: 700; margin-bottom: 20px; position: relative; z-index: 1; }
        .blog-hero h1 { font-size: clamp(30px, 5vw, 54px); font-weight: 900; color: #fff; margin-bottom: 16px; position: relative; z-index: 1; }
        .blog-hero p { max-width: 560px; margin: 0 auto 32px; color: rgba(255,255,255,0.78); font-size: 16px; line-height: 2; position: relative; z-index: 1; }
        .blog-hero-logo-ring { width: 148px; height: 148px; border-radius: 50%; background: rgba(255,255,255,0.12); border: 2px solid rgba(255,255,255,0.2); display: flex; align-items: center; justify-content: center; backdrop-filter: blur(8px); margin: 0 auto; }
        .blog-wave { display: block; width: 100%; overflow: hidden; line-height: 0; margin-top: -2px; }
        .blog-wave svg { display: block; width: 100%; }
        .blog-breadcrumb { max-width: 1080px; margin: 0 auto; padding: 20px 24px; display: flex; align-items: center; gap: 8px; font-size: 13px; color: #a07060; border-bottom: 1.5px solid rgba(143,29,29,0.08); }
        .blog-breadcrumb a { color: #8f1d1d; text-decoration: none; font-weight: 600; }
        .blog-breadcrumb a:hover { text-decoration: underline; }
        .blog-breadcrumb-sep { color: #cbb0a0; }
        .blog-container { max-width: 1080px; margin: 0 auto; padding: 40px 24px 80px; }
      ...
      <section className="blog-hero">
        <div className="blog-hero-badge">{blogT.heroBadge}</div>
        <h1>{blogT.heroTitle}</h1>
        <p>{blogT.heroText}</p>
      ...
      <nav className="blog-breadcrumb">
        ...
        <span className="blog-breadcrumb-sep">›</span>
        <span>{commonT.page} {currentPage}</span>
      </nav>
      ...
      <div className="blog-container">
        ...
        <div className="social-section">
          ...
          <div className="social-icons">
            {/* same social icons as above – keep original */}
