...
export async function generateMetadata({ params }: BlogPostPageProps) {
  ...
  return {
    title: `${post.title} | چیپس ذرت ترددیلا`,
...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
  ...
  const categories = [
    { name: "اخبار", slug: "news", count: 1 },
    { name: "بدانیم", slug: "badanim", count: 4 },
    { name: "سالم بخوریم", slug: "healthy-eating", count: 1 },
    { name: "طرز تهیه دیپ", slug: "recipe-dip", count: 3 },
    { name: "طرز تهیه سس", slug: "recipe-sauce", count: 2 },
    { name: "طرز تهیه غذا", slug: "recipe-food", count: 5 },
  ];
    ...
    <div className="bg-white">
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-[#fbf5ec] to-white py-12 md:py-16">
        <div className="container mx-auto px-4 md:px-6">
          <div className="text-center">
            <h1 className="text-3xl font-black text-[#8f1d1d] md:text-4xl lg:text-5xl">
              {post.title}
            </h1>
          </div>
        </div>
      ...
      <div className="container mx-auto px-4 py-4 md:px-6">
        <div className="text-sm text-neutral-500">
          <Link href="/" className="hover:text-[#8f1d1d]">
            چیپس ذرت ترددیلا
          </Link>{" "}
          &gt;{" "}
          <Link href="/blog" className="hover:text-[#8f1d1d]">
            وبلاگ
          </Link>{" "}
          &gt;{" "}
          <Link
            href={`/blog/category/${post.categorySlug}`}
            className="hover:text-[#8f1d1d]"
          >
            {post.category}
          </Link>{" "}
      ...
      <div className="container mx-auto px-4 py-8 md:px-6">
        <div className="flex flex-col gap-8 lg:flex-row">
          ...
          <article className="lg:w-2/3">
            {/* Meta Info */}
            <div className="mb-6 flex flex-wrap items-center gap-4 border-b border-neutral-200 pb-4 text-sm text-neutral-500">
              <span>📅 {post.date}</span>
              <span>📁 دسته: {post.category}</span>
              <span>✍️ نویسنده: {post.author}</span>
            </div>
              ...
              <div className="flex gap-3">
                <a
                  href={`https://t.me/share/url?url=${encodeURIComponent(
                    `https://tordillafood.com/blog/${post.slug}`
                  )}&text=${encodeURIComponent(post.title)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="rounded-full bg-[#29A9E1] px-4 py-2 text-sm text-white transition hover:bg-[#1e7eaa]"
                >
                  تلگرام
                </a>
                <a
                  href={`https://wa.me/?text=${encodeURIComponent(
                    `${post.title} - https://tordillafood.com/blog/${post.slug}`
                  )}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="rounded-full bg-[#25D366] px-4 py-2 text-sm text-white transition hover:bg-[#1a9248]"
                >
                  واتساپ
                </a>
              </div>
          ...
          <aside className="lg:w-1/3">
            ...
            <div className="mb-8 rounded-2xl border border-neutral-200 bg-white p-6 shadow-md">
              <h3 className="mb-4 text-xl font-bold text-[#8f1d1d]">
                دسته بندی
              </h3>
                ...
                {categories.map((cat) => (
                  <li key={cat.slug}>
                    <Link
                      ...
                      className="flex items-center justify-between rounded-lg p-2 text-neutral-700 transition hover:bg-[#8f1d1d]/5 hover:text-[#8f1d1d]"
                    >
                      <span>{cat.name}</span>
            ...
            <div className="mb-8 overflow-hidden rounded-2xl shadow-md">
              <div className="relative h-48 w-full">
                <Image
                  src="https://tordillafood.com/wp-content/uploads/instapic.jpg"
              ...
              <div className="bg-[#8f1d1d] p-4 text-center text-white">
                <p className="mb-3">اینستاگرام ترددیلا را دنبال کنید!</p>
                <a
                  href="https://instagram.com/tordillachips/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-block rounded-full bg-[#39a845] px-6 py-2 text-sm font-bold transition hover:bg-[#2f8d39]"
                >
                  دنبال کردن
                </a>
              </div>
      ...
      <section className="bg-[#f6f1ec] py-12">
        <div className="container mx-auto px-4 md:px-6">
          <h3 className="mb-8 text-center text-2xl font-bold text-[#8f1d1d] md:text-3xl">
            مطالب مرتبط در بلاگ ترددیلا
          </h3>
      ...
      <section className="container mx-auto px-4 py-12 md:px-6">
        <h3 className="mb-6 text-xl font-bold text-[#8f1d1d]">نظرات</h3>
        <div className="rounded-2xl border border-neutral-200 bg-white p-6">
          <p className="mb-4 text-neutral-600">دیدگاه خود را بیان کنید.</p>
          <form className="space-y-4">
            <textarea
              rows={5}
              placeholder="دیدگاه یا پاسخ شما..."
              className="w-full rounded-xl border border-neutral-300 p-3 focus:border-[#8f1d1d] focus:outline-none focus:ring-2 focus:ring-[#8f1d1d]/20"
            />
            <div className="grid gap-4 sm:grid-cols-2">
              <input
                type="text"
                placeholder="نام"
                className="rounded-xl border border-neutral-300 p-3 focus:border-[#8f1d1d] focus:outline-none"
              />
              <input
                type="email"
                placeholder="ایمیل"
            ...
            <button
              type="submit"
              className="rounded-full bg-[#8f1d1d] px-6 py-2 text-white transition hover:bg-[#6b1616]"
            >
              ارسال نظر
            </button>
          </form>
